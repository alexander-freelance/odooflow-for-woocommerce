...
class OdooFlow {
    ...
    /** @var array Cache for Odoo attribute IDs by name */
    protected $attribute_cache = array();

    /** @var array Cache for Odoo attribute value IDs */
    protected $attribute_value_cache = array();

    /** @var array Cache for product.template.attribute.value IDs */
    protected $ptav_cache = array();

    /**
     * Helper to encode XML-RPC requests in UTF-8
     */
    private function xmlrpc_encode_utf8($method, $params) {
    ...
    public function ajax_list_odoo_modules() {
        ...
        $auth_request = $this->xmlrpc_encode_utf8('authenticate', array(
            $database,
            $username,
            $api_key,
            array()
    ...
    public function ajax_get_woo_products() {
        ...
        // Query WooCommerce products with simple pagination
        $args = array(
            'post_type'      => 'product',
            'post_status'    => 'publish',
            'posts_per_page' => 100,
                ...
                if ($product->is_type('variable')) {
                    ...
                    foreach ($product->get_children() as $child_id) {
                        ...
                        if ($variation && $variation->get_regular_price() !== '') {
                            $prices[] = (float) $variation->get_regular_price();
                        }
                    }
                    if (!empty($prices)) {
                        $price = min($prices);
    ...
    private function get_odoo_category_id($name, $odoo_url, $database, $uid, $api_key) {
        ...
        $cat_id = (is_array($ids) && isset($ids[0])) ? $ids[0] : null;
    ...
    /**
     * Get (or create) an attribute in Odoo and return its ID
     */
    private function get_odoo_attribute_id($name, $odoo_url, $database, $uid, $api_key) {
        ...
        $attr_id = (is_array($ids) && isset($ids[0])) ? $ids[0] : null;
        ...
        if ($attr_id) {
            $this->attribute_cache[$name] = $attr_id;
        }
    ...
    private function get_odoo_attribute_value_id($attribute_id, $name, $odoo_url, $database, $uid, $api_key) {
        ...
        $search_req = $this->xmlrpc_encode_utf8('execute_kw', array(
            ...
            'search',
            array(array(array('name', '=', $name), array('attribute_id', '=', $attribute_id))),
            0,
            1
        ));
        ...
        $ids = is_wp_error($resp) ? array() : xmlrpc_decode(wp_remote_retrieve_body($resp));
        $val_id = (is_array($ids) && isset($ids[0])) ? $ids[0] : null;
        ...
        if ($val_id) {
            $this->attribute_value_cache[$cache_key] = $val_id;
        }
    ...
    /**
     * Get (or create) a product.template.attribute.value ID
     */
    private function get_odoo_ptav_id($template_id, $attribute_id, $value_id, $odoo_url, $database, $uid, $api_key, $price_extra = null) {
        $cache_key = $template_id . ':' . $attribute_id . ':' . $value_id;
        if (isset($this->ptav_cache[$cache_key])) {
            return $this->ptav_cache[$cache_key];
        }
        ...
        $search_req = $this->xmlrpc_encode_utf8('execute_kw', array(
            ...
            array('fields' => array('id', 'price_extra'), 'limit' => 1)
        ));
        ...
        $rows = is_wp_error($resp) ? array() : xmlrpc_decode(wp_remote_retrieve_body($resp));
        $ptav_id = (is_array($rows) && !empty($rows)) ? $rows[0]['id'] : null;
        $current_extra = (is_array($rows) && !empty($rows)) ? floatval($rows[0]['price_extra']) : null;
        ...
        if (!$ptav_id) {
            ...
            $ptav_id = is_wp_error($create_resp) ? null : xmlrpc_decode(wp_remote_retrieve_body($create_resp));
            if (!is_numeric($ptav_id)) {
                $ptav_id = null;
            }
        }

        if ($ptav_id) {
            $this->ptav_cache[$cache_key] = $ptav_id;
            if ($price_extra !== null && $current_extra !== $price_extra) {
                $update_req = $this->xmlrpc_encode_utf8('execute_kw', array(
                    $database,
                    $uid,
                    $api_key,
                    'product.template.attribute.value',
                    'write',
                    array(array($ptav_id), array('price_extra' => $price_extra))
                ));
                wp_remote_post($object_ep, [
                    'body' => $update_req,
                    'headers' => ['Content-Type' => 'text/xml'],
                    'timeout' => 30,
                    'sslverify' => false
    ...
    private function export_product_to_odoo($product, $selected_fields) {
        ...
        if ($product->is_type('variable')) {
            ...
            foreach ($attributes as $attr) {
                ...
                $attr_id = $this->get_odoo_attribute_id($attr_label, $odoo_url, $database, $uid, $api_key);
                if (!$attr_id) {
                    continue;
                }
                ...
                foreach ($options as $opt) {
                    if ($attr->is_taxonomy()) {
                        $term = get_term_by('slug', $opt, $attr->get_name());
                        $opt_name = $term ? $term->name : $opt;
                        $map_key = $opt;
                    } else {
                        $opt_name = $opt;
                        $map_key = $opt;
                    ...
                    $val_id = $this->get_odoo_attribute_value_id($attr_id, $opt_name, $odoo_url, $database, $uid, $api_key);
                    if ($val_id) {
                        $value_ids[] = $val_id;
                        $attr_value_map[$attr->get_name()][$map_key] = array(
                            'attr_id' => $attr_id,
                            'val_id'  => $val_id
                        );
                    }
            ...
            foreach ($product->get_children() as $child_id) {
                ...
                if ($v && $v->get_regular_price() !== '') {
                    $p = floatval($v->get_regular_price());
                    if ($base_price === null || $p < $base_price) {
                        $base_price = $p;
                    }
                }
            }
            if ($base_price !== null) {
                $product_data['list_price'] = $base_price;
            }
        }
        ...
        $search_request = $this->xmlrpc_encode_utf8('execute_kw', array(
            $database,
            $uid,
            $api_key,
            'product.template',
        ...
        if (is_array($search_result) && !empty($search_result)) {
            ...
            // Avoid duplicating attribute lines when updating an existing template
            if (isset($product_data['attribute_line_ids'])) {
                unset($product_data['attribute_line_ids']);
            }

            // Update existing product
            $update_request = $this->xmlrpc_encode_utf8('execute_kw', array(
                $database,
                $uid,
                $api_key,
                'product.template',
            ...
            // Create new product
            $create_request = $this->xmlrpc_encode_utf8('execute_kw', array(
                $database,
                $uid,
                $api_key,
                'product.template',
        ...
        if ($odoo_id && $product->is_type('variable')) {
            ...
            foreach ($variation_ids as $variation_id) {
                ...
                if (empty($v_sku)) {
                    $v_sku = get_post_meta($variation->get_id(), '_sku', true);
                }
                if (empty($v_sku)) {
                    $v_sku = $parent_sku . '-' . $variation->get_id();
                }
                $v_data = array(
                ...
                $ptav_ids = array();
                $var_price = floatval($variation->get_regular_price());
                $v_data['list_price'] = $var_price;
                foreach ($var_attrs as $attr_slug => $val_slug) {
                    $attr_id = null;
                    $val_id = null;
                    if (isset($attr_value_map[$attr_slug][$val_slug])) {
                        $attr_id = $attr_value_map[$attr_slug][$val_slug]['attr_id'];
                        $val_id  = $attr_value_map[$attr_slug][$val_slug]['val_id'];
                    } else {
                        ...
                        if ($attr_id) {
                            if (taxonomy_exists($attr_slug)) {
                                $term = get_term_by('slug', $val_slug, $attr_slug);
                                $vname = $term ? $term->name : $val_slug;
                            } else {
                                $vname = $val_slug;
                            }
                            $val_id = $this->get_odoo_attribute_value_id($attr_id, $vname, $odoo_url, $database, $uid, $api_key);
                        }
                    }

                    if ($attr_id && $val_id) {
                        $ptav = $this->get_odoo_ptav_id(
                            $odoo_id,
                            $attr_id,
                            $val_id,
                            $odoo_url,
                ...
                if (!empty($ptav_ids)) {
                    $v_data['product_template_attribute_value_ids'] = array(array(6, 0, $ptav_ids));
                }
                ...
                $v_search_req = $this->xmlrpc_encode_utf8('execute_kw', array(
                        $database,
                        $uid,
                        $api_key,
                        'product.product',
                        'search_read',
                        array(array(array('default_code', '=', $v_sku))),
                        array('fields' => array('id'))
                    ));
                ...
                if (is_array($v_search) && !empty($v_search)) {
                    ...
                    $v_update_req = $this->xmlrpc_encode_utf8('execute_kw', array(
                            $database,
                            $uid,
                            $api_key,
                            'product.product',
                    ...
                    $v_create_req = $this->xmlrpc_encode_utf8('execute_kw', array(
                            $database,
                            $uid,
                            $api_key,
                            'product.product',
            ...
            if (!empty($parent_sku)) {
                $sku_update = $this->xmlrpc_encode_utf8('execute_kw', array(
                    $database,
                    $uid,
                    $api_key,
                    'product.template',
                    'write',
                    array(array($odoo_id), array('default_code' => $parent_sku))
                ));
                wp_remote_post(rtrim($odoo_url, '/') . '/xmlrpc/2/object', [
                    'body' => $sku_update,
                    'headers' => ['Content-Type' => 'text/xml'],
    ...
    private function oflow_add_col_fields( $source, array $payload,
        ...
        $id_code_raw = $get_meta( 'tipo_identificacion' );
        $id_code  = strtoupper( $id_code_raw ); // 13,22,31... o ID numérico
        ...
        if ( $id_code !== '' ) {
            if ( is_numeric( $id_code_raw ) ) {
                $payload['l10n_latam_identification_type_id'] = (int) $id_code_raw;
            } else {
    ...
    public function ajax_export_woo_customers() {
        ...
        try {
            ...
            foreach ($customer_ids as $customer_id) {
                ...
                // Prepare customer data with geographic mapping
                $country_id = $this->get_country_id(
                    $wc_customer->get_billing_country(),
                    $database, $uid, $api_key, $object_endpoint
                );
                $state_id = $this->get_state_id(
                    $wc_customer->get_billing_state(),
                    $country_id, $database, $uid, $api_key, $object_endpoint
                );
                $city_id = $this->get_city_id(
                    $wc_customer->get_billing_city(),
                    $state_id, $database, $uid, $api_key, $object_endpoint
                );
    ...
    private function get_or_create_odoo_product($product) {
        ...
        $result = $this->create_odoo_product($product);
     ...
     */
    private function create_odoo_product($product) {
        if (!$product) {
            return new WP_Error('invalid_product', __('Invalid product.', 'odooflow'));
        }

        $fields = array('name', 'default_code', 'list_price', 'description', 'qty_available', 'weight');

        $result = $this->export_product_to_odoo($product, $fields);

        if (is_wp_error($result)) {
            return $result;
        }

        return $result['odoo_id'] ?? 0;
    }
    ...
    private function create_odoo_customer($customer_data) {
        ...
        if ($email !== '') {
            ...
            if (is_wp_error($search_response)) {
                return new WP_Error('search_error', __('Error searching for customer in Odoo.', 'odooflow'));
            }
            ...
            if (!empty($existing_ids)) {
                ...
                if (is_wp_error($update_response)) {
                    return new WP_Error('update_error', __('Error updating customer in Odoo.', 'odooflow'));
                }
     ...
     */
    private function get_country_id($country_code, $db, $uid, $api_key, $object_ep) {
        static $cache = [];
        ...
        $result = is_wp_error($response)
            ? []
            : xmlrpc_decode(wp_remote_retrieve_body($response));
    ...
    /**
     * Get state ID from Odoo
     */
    private function get_state_id($state_code, $country_id, $db, $uid, $api_key, $object_ep) {
        static $cache = [];
        $code = strtoupper(trim($state_code));
        $key  = $country_id . ':' . $code;
        if (isset($cache[$key]) || $code === '' || $country_id <= 0) {
            return $cache[$key] ?? 0;
        }
    ...
    /**
     * Get city ID from Odoo
     */
    private function get_city_id($city_name, $state_id, $db, $uid, $api_key, $object_ep) {
        static $cache = [];
        $key = $state_id . ':' . strtolower($city_name);
        if (isset($cache[$key]) || $state_id <= 0 || $city_name === '') {
            return $cache[$key] ?? 0;
        }
