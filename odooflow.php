...
class OdooFlow {
    ...
    private function export_product_to_odoo($product, $selected_fields) {
        ...
        if ($product->is_type('variable')) {
            ...
            if ($min_price !== null) {
                $product_data['list_price'] = $min_price;
            }

            $price_extra_map = array();
            foreach ($variation_data as $vinfo) {
                $diff = floatval($vinfo['price']) - $min_price;
                if ($diff == 0) continue;
                $diff_attrs = array();
        ...
        if ($odoo_id && $product->is_type('variable')) {
            ...
            foreach ($variation_ids as $variation_id) {
                ...
                $var_diff = floatval($variation->get_regular_price()) - $base_price;
                foreach ($var_attrs as $attr_slug => $val_slug) {
                    ...
                    if ($attr_id && $val_id) {
                        ...
                        } elseif ($var_diff > 0 && count($var_attrs) === 1) {
                            $extra = $var_diff;
                        }
                        $ptav = $this->get_odoo_ptav_id($odoo_id, $attr_id, $val_id, $odoo_url, $database, $uid, $api_key, $extra);
                        if ($ptav) {
                            $ptav_ids[] = $ptav;
                        }
                    }
