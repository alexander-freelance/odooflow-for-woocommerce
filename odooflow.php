...
class OdooFlow {
    ...
    private function oflow_add_col_fields( $source, array $payload,
        ...
        if ( $id_code ) {
            ...
            $odoo_code = strtolower( $map[ $id_code ] ?? $id_code );

            if ( ! isset( $cache[ $odoo_code ] ) ) {

                // 🗄️  Buscamos el ID usando el campo 'code' de Odoo
                ...
                $cache[ $odoo_code ] = is_array( $ids ) && $ids ? $ids[0] : null;
            }

            if ( $cache[ $odoo_code ] ) {
                $payload['l10n_latam_identification_type_id'] = $cache[ $odoo_code ];
            }
        }
