    ...
    private function export_product_to_odoo($product, $selected_fields) {
        ...
        if ($product->is_type('variable')) {
            ...
            foreach ($product->get_children() as $child_id) {
                ...
                if ($v && $v->get_regular_price() !== '') {
                    ...
                    $variation_data[] = array('price' => $p, 'attrs' => $attrs);
            ...
            $price_extra_map = array();
            if (isset($variation_data[0])) {
                foreach ($variation_data as $vinfo) {
                    $diff = floatval($vinfo['price']) - $base_price;
                    if ($diff == 0) continue;
                    $diff_attrs = array();
                    foreach ($vinfo['attrs'] as $aslug => $avalue) {
                        if (isset($base_attrs[$aslug]) && $base_attrs[$aslug] !== $avalue) {
                            $diff_attrs[$aslug] = $avalue;
                        }
                    }
                    if (count($diff_attrs) == 1) {
                        foreach ($diff_attrs as $ds => $dv) {
                            if (!isset($price_extra_map[$ds][$dv])) {
                                $price_extra_map[$ds][$dv] = $diff;
                            }
                        }
                    }
                }
        ...
        if ($odoo_id && $product->is_type('variable')) {
            ...
            foreach ($variation_ids as $variation_id) {
                ...
                $ptav_ids = array();
                $base_price = isset($product_data['list_price']) ? floatval($product_data['list_price']) : 0;
                ...
                foreach ($var_attrs as $attr_slug => $val_slug) {
                    ...
                    if ($attr_id && $val_id) {
                        $extra = null;
                        if (isset($price_extra_map[$attr_slug][$val_slug])) {
                            $extra = $price_extra_map[$attr_slug][$val_slug];
                        } else {
                            $extra = $var_price - $base_price;
                        }
                        $ptav = $this->get_odoo_ptav_id($odoo_id, $attr_id, $val_id, $odoo_url, $database, $uid, $api_key, $extra != 0 ? $extra : null);
                        if ($ptav) {
                            $ptav_ids[] = $ptav;
                        }
                    }
