...
class OdooFlow {
    ...
    private function get_or_create_odoo_customer($order, $database, $uid, $api_key) {
        ...
        if ($email) {
            ...
            if (is_wp_error($resp)) {
                return new WP_Error('search_error', __('Error searching customer in Odoo.', 'odooflow'));
            }
        ...
        if ($existing_id) {
            $update_req = xmlrpc_encode_request('execute_kw', array(
                $database, $uid, $api_key,
                'res.partner', 'write',
                array(array($existing_id), $customer_data)
            ));
    ...
    private function get_odoo_tax_id($wc_tax_id) {
        ...
        $rate = WC_Tax::get_rate($wc_tax_id);
        if (!$rate) {
            return 0;
        }
        ...
        $search_req = xmlrpc_encode_request('execute_kw', array(
            $database, $uid, $api_key,
            'account.tax', 'search',
            array(array(array('name', 'ilike', $rate['label']))),
            0, 1
        ));
