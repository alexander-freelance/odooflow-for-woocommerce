...
5. **City** → `city_id` (via `lookup_city_id()`; falls back to `city` text from meta `billing_ciudad` or `billing_city`)
...
| `lookup_city_id()`    | ✔ implemented | Search `res.city` by name and state |
